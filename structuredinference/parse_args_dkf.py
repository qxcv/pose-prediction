...
parser.add_argument('-dset', '--dataset', action='store', default='', help='Dataset')
...
parser.add_argument('-vm', '--var_model', action='store', default='R', help='Variational Model', choices=['L', 'LR', 'R'])
parser.add_argument('-infm', '--inference_model', action='store', default='structured', help='Inference Model', choices=['mean_field', 'structured'])
...
parser.add_argument('-ttype', '--transition_type', action='store', default='simple_gated', help='Layers in transition fxn', choices=['mlp', 'simple_gated'])
...
parser.add_argument('-etype', '--emission_type', action='store', default='mlp', help='Type of emission fxn', choices=['mlp', 'conditional', 'res'])
...
parser.add_argument('-ischeme', '--init_scheme', action='store', default='uniform', help='Type of initialization for weights', choices=['uniform', 'normal', 'xavier', 'he', 'orthogonal'])
parser.add_argument('-nl', '--nonlinearity', action='store', default='relu', help='Nonlinarity', choices=['relu', 'tanh', 'softplus', 'maxout', 'elu'])
...
parser.add_argument('-repK', '--replicate_K', action='store', default=1, help='Number of samples used for the variational bound. Created by replicating the batch', type=int)
...
#Regularization
parser.add_argument('-reg', '--reg_type', action='store', default='l2', help='Type of regularization', choices=['l1', 'l2'])
parser.add_argument('-rv', '--reg_value', action='store', default=0.05, help='Amount of regularization', type=float)
parser.add_argument('-rspec', '--reg_spec', action='store', default='_', help='String to match parameters (Default is generative model)')

#Save/load
parser.add_argument('-debug', '--debug', action='store_true', help='Debug')
parser.add_argument('-uid', '--unique_id', action='store', default='uid', help='Unique Identifier')
parser.add_argument('-seed', '--seed', action='store', default=1, help='Random Seed', type=int)
parser.add_argument('-dir', '--savedir', action='store', default='./chkpt', help='Prefix for savedir')
parser.add_argument('-ep', '--epochs', action='store', default=2000, help='MaxEpochs', type=int)
parser.add_argument('-reload', '--reloadFile', action='store', default='./NOSUCHFILE', help='Reload from saved model')
parser.add_argument('-params', '--paramFile', action='store', default='./NOSUCHFILE', help='Reload parameters from saved model')
parser.add_argument('-sfreq', '--savefreq', action='store', default=25, help='Frequency of saving', type=int)
parser.add_argument('-cond', '--use-cond', action='store_true', default=False, help='Use conditioning input (if possible)')
